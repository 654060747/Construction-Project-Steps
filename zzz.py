...
def css_get(doc):
    ...
    i=0
    for item in svg_links:
        svg_link = item.replace(")", "").replace("(", "http:")
        if i == 0:
            dict_avg_text, list_svg_y = svg_new_text(svg_link)
            print(svg_link)
        else:
            dict_avg_text, list_svg_y = svg_text(svg_link)
        i = i + 1
        # dict_css = css_dict(background_link.text)
    # [0].replace(")", "").replace("(", "http:")
...
# 2-字体库链接
def svg_text(url):
    html = requests.get(url, headers=header_css)
    print(html.text)
...
# new 2-字体库链接
def svg_new_text(url):
    html = requests.get(url, headers=header_css)
    print(html.text)
    dict_svg, list_y = svg_new_dict(html.text)
    print(dict_svg)
    print(list_y)
    return dict_svg, list_y
...
# new 3-生成svg字库字典 190302
def svg_new_dict(csv_html):
    # svg_text_r = r'<textPath xlink:href="(.*?)" textLength="(.*?)">(.*?)</textPath>'
    svg_text_r = r'<text x="(.*?)" y="(.*?)">(.*?)</text>'
    # <textPath xlink:href="#1" textLength="588">湛徽名盐北春红凰二创定城内农七岛场乐头湾解襄康永常都哈开利胜陕深区通学远昌进安威凤韶</textPath>
    # <text x="0" y="31">线政妻贸捷付滥眠中旬越向锅猫爪造签捉请咬攻填关漂难舍虾乐商醋剧亏舞波刻使老纸急轧仔毕</text>
    svg_text_re = re.findall(svg_text_r, csv_html)
    print(svg_text_re)
    dict_avg = {}
    # 生成svg加密字体库字典
